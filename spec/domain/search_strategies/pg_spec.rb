  ...
  describe '#query_people' do
    ...
    context 'as admin' do
      ...
      it 'finds accessible person' do
        result = strategy(@bg_leader.last_name[0..5]).query_people

        expect(result).to include(@bg_leader)
      end

      it 'finds accessible person with two terms' do
        result = strategy("#{@bg_leader.last_name[0..5]} #{@bg_leader.first_name[0..3]}").query_people
